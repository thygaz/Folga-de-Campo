...
def calcular_status(colab):
    ...
    if not colab.ultima_folga_fim: return {"texto": "Definir (Lance a última folga)", "classe": "bg-secondary", "dias_trabalhados": 0}
...
def get_recursos():
    ...
    lista = []
    for c in colaboradores:
        # Calculamos o status aqui também para exibir na timeline
        st = calcular_status(c)
        lista.append({
            'id': c.id, 
            'title': c.nome, # Nome completo
    ...
    if mostrar_trabalho:
        ...
        for c in colabs:
            if c.ultima_folga_fim:
                ...
                if proxima_folga: end_work = proxima_folga - timedelta(days=1)
                else:
                ...
                if end_work >= start_work:
                    ...
                    if duracao > 60: bg, txt, bd = '#ffebee', '#c62828', '#ef9a9a'
                    ...
                    eventos.append({
                        ...
                        'extendedProps': {'tipo_evento': 'trabalho'}, 'display': 'block' 
                    })
    return jsonify(eventos)

# --- ROTAS DE AÇÃO (MANTIDAS IGUAIS) ---
...
def adicionar_folga():
    ...
    if d_inicio <= get_hoje():
        if not colab.ultima_folga_fim or d_fim > colab.ultima_folga_fim: colab.ultima_folga_fim = d_fim
    db.session.add(nova)
    db.session.commit()
